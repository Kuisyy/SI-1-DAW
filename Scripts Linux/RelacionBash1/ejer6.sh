...
if [id "$loginUsuario" &>/dev/null] 
    then
    echo "El login $loginUsuario ya está ocupado. Elige otro." #El &>/dev/null al final redirige tanto la salida estándar (stdout) como la salida de error estándar (stderr) a la nada para que no salga nada en la temrinal
    exit 1
fi
